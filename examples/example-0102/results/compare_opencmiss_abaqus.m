...
REFINEMENT = 'MEDIUM'; % COARSE (x); MEDIUM (x); FINE
DIMENSION = '3D'; % 2D; 3D (x)
...
%% PLOTTING
figure(1)
title('Displacement Comparison between OpenCMISS and ABAQUS')
plot(x_vec,abaq_topSurf_values(:,dispOfInterest),'-r')
plot(x_vec,cmiss_topSurf_values)
xlabel('reference horizontal distance (mm)')
ylabel('comparison displacement (mm)')
