...
# first compare numerical solutions
...
for s in np.arange(0, 2, 1):
    ...
    for i in np.arange(1, 3, 1):
        ...
        for r in np.arange(0, NumberOfElements.shape[0], 1):
            ...
            ####################################################################
            # read reference data
            foldername  = "reference/cheart/l4x2x0_n"+str(nx)+"x"+str(ny)+"x"+str(nz)+"_i"+str(i)+"/"
            ...
            ivals0       = 0.0 * cvals
            foldername  = "reference/iron/l4x2x0_n"+str(nx)+"x"+str(ny)+"x"+str(nz)+"_i"+str(i)+"_s"+str(s)+"/"
            ...
            # read current_run
            ivals       = 0.0 * cvals
            foldername  = "current_run/l4x2x0_n"+str(nx)+"x"+str(ny)+"x"+str(nz)+"_i"+str(i)+"_s"+str(s)+"/"
            filename    = foldername + "Example.part0.exnode"
            linecount = 0
            matched   = 0
            with open(filename) as f:
...
if False:
    # now, compare with analytical solution
    NumberOfElements = np.zeros((1,3), dtype=int)
    NumberOfElements[0][0] = 100
    NumberOfElements[0][1] =  50
    NumberOfElements[0][2] =   0
    ...
    for s in np.arange(0, 2, 1):
        ...
        for i in np.arange(1, 3, 1):
            ...
            for r in np.arange(0, NumberOfElements.shape[0], 1):
                ...
                ####################################################################
                # read reference data
                foldername  = "reference/analytical/l4x2x0_n"+str(nx)+"x"+str(ny)+"x"+str(nz)+"_i"+str(i)+"/"
                filename    = foldername + "SpaceVariable-1.D"
                # load analytical space variable, skip first line
                cxyz        = np.loadtxt(filename, dtype=float, skiprows=1)
                filename    = foldername + "AnalyticalSolution-0.D"
                # load analytical scalar variable, skip first line
                cvals       = np.loadtxt(filename, dtype=float, skiprows=1)
                ####################################################################
                # read current_run
                ivals       = 0.0 * cvals
                ...
                ####################################################################
                # compute difference
                l2diff_ci   = np.linalg.norm(cvals-ivals, 2) / np.sqrt(cxyz.shape[0])
                if (l2diff_ci > tolu):
                    status = filename+"       | Analytical - Iron |_2 = "+str(l2diff_ci)
                    print status
                    if (NumberOfFailedTests == 0):
                        failedtests_file.write("Failed tests:\n")
                    failedtests_file.write(status+"\n")
