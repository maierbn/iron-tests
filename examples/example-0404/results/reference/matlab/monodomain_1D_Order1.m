...
%% THIS HAS NEVER BEEN USED FOR THE PHD THESIS AND IS COPIED FROM "COMPUATATIONAL METHODS IN BIOMECHANICS" ASSINGMENT 3 %%% 
  ...
  %------------------------------------------------------------------------ 
  %Initial stimulation
  %tspan = 0:1/ num_of_steps_ode:1;
  %[V_m,ALL_STATES]=InitialStimulation(1,num_of_points,ALL_STATES,CONSTANTS,i_Stim,tspan);
  %name=['vm_' num2str(0) '.csv'];
  %dlmwrite(name,V_m,';');
  %------------------------------------------------------------------------
  ...
  for time = 1:num_of_steps_pde
    ...
    name=['vm_' num2str((time-1)*time_step_pde) '.csv'];
    dlmwrite(name,V_m,';');
  ...
  tt = linspace(time_step_pde,t_end,num_of_steps_pde);
...
%not necessary
function [Vm,ALL_STATES]=InitialStimulation(time,num_of_points,ALL_STATES,CONSTANTS,i_Stim,tspan)
  [Vm,ALL_STATES]=SolveCellular(1,time,num_of_points,ALL_STATES,CONSTANTS,i_Stim,tspan);
end
