...
!  CALL cmfe_DiagnosticsSetOn(CMFE_IN_DIAG_TYPE,[1,2,3,4,5],"Diagnostics",["DOMAIN_MAPPINGS_LOCAL_FROM_GLOBAL_CALCULATE"],Err)
  ...
  !Set the left surface nodes to linearly increase in y from 0 to 15
  DO NodeIdx=1,SIZE(LeftSurfaceNodes,1)
    ...
    IF(NodeDomain==ComputationalNodeNumber) THEN
      ...
      DirichletValue=y/HEIGHT*15.0_CMISSRP
  ...
  DO NodeIdx=1,SIZE(RightSurfaceNodes,1)
    ...
    IF(NodeDomain==ComputationalNodeNumber) THEN
      CALL cmfe_Field_ParameterSetGetNode(GeometricField,CMFE_FIELD_U_VARIABLE_TYPE,1, &
        & 1,1,NodeNumber,2,y,Err)
      DirichletValue=25.0_CMISSRP-y/HEIGHT*18.0_CMISSRP
      CALL cmfe_BoundaryConditions_SetNode( &
        & BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber,1, &
        & CMFE_BOUNDARY_CONDITION_FIXED,DirichletValue,Err)
    ENDIF
