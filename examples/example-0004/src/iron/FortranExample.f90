...
#include "mpif.h"
  ...
  INTEGER(CMISSIntg)                :: NodeIdx,NodeNumber,LastNodeNumber,NodeDomain
  ...
  REAL(CMISSRP)                     :: DirichletValue,x,y
...
!  CALL cmfe_DiagnosticsSetOn(CMFE_IN_DIAG_TYPE,[1,2,3,4,5],"Diagnostics",["DOMAIN_MAPPINGS_LOCAL_FROM_GLOBAL_CALCULATE"],Err)
  ...
  CALL cmfe_GeneratedMesh_SurfaceGet( &
    & GeneratedMesh,CMFE_GENERATED_MESH_REGULAR_LEFT_SURFACE, &
    & LeftSurfaceNodes,LeftNormalXi,Err)
  CALL cmfe_GeneratedMesh_SurfaceGet( &
    & GeneratedMesh,CMFE_GENERATED_MESH_REGULAR_RIGHT_SURFACE, &
    & RightSurfaceNodes,RightNormalXi,Err)
  CALL cmfe_GeneratedMesh_SurfaceGet( &
    & GeneratedMesh,CMFE_GENERATED_MESH_REGULAR_FRONT_SURFACE, &
    & FrontSurfaceNodes,FrontNormalXi,Err)
  CALL cmfe_GeneratedMesh_SurfaceGet( &
    & GeneratedMesh,CMFE_GENERATED_MESH_REGULAR_BACK_SURFACE, &
    & BackSurfaceNodes,BackNormalXi,Err)
  !Set all surface nodes to analytical solution
  DO NodeIdx=1,SIZE(LeftSurfaceNodes,1)
    ...
    IF(NodeDomain==ComputationalNodeNumber) THEN
      ...
      CALL AnalyticalSolution(x,y,DirichletValue)
      CALL cmfe_BoundaryConditions_SetNode( &
        & BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber,1, &
  ...
  DO NodeIdx=1,SIZE(FrontSurfaceNodes,1)
    ...
    ENDIF
  ENDDO
  DO NodeIdx=1,SIZE(BackSurfaceNodes,1)
    NodeNumber=BackSurfaceNodes(NodeIdx)
    CALL cmfe_Decomposition_NodeDomainGet(Decomposition,NodeNumber,1,NodeDomain,Err)
    IF(NodeDomain==ComputationalNodeNumber) THEN
      CALL cmfe_Field_ParameterSetGetNode(GeometricField,CMFE_FIELD_U_VARIABLE_TYPE,1, &
        & 1,1,NodeNumber,1,x,Err)
  ...
  IF(NUMBER_GLOBAL_X_ELEMENTS==1000) THEN
    WRITE(filename, "(A21,I1,A1,I1,A1,I1,A2,I4,A1,I3,A1,I1,A2,I1,A2,I1,A8)") &
  ...
  ELSEIF(NUMBER_GLOBAL_X_ELEMENTS==100) THEN
    WRITE(filename, "(A21,I1,A1,I1,A1,I1,A2,I3,A1,I2,A1,I1,A2,I1,A2,I1,A8)") &
      & "results/current_run/l", &
      & INT(WIDTH),"x",INT(HEIGHT),"x",INT(LENGTH), &
      & "_n", &
      & NUMBER_GLOBAL_X_ELEMENTS,"x",NUMBER_GLOBAL_Y_ELEMENTS,"x",NUMBER_GLOBAL_Z_ELEMENTS, &
...
CONTAINS
  ...
  SUBROUTINE AnalyticalSolution(x,y,Value)

    REAL(CMISSRP),  INTENT(IN)  :: x
    REAL(CMISSRP),  INTENT(IN)  :: y
    REAL(CMISSRP),  INTENT(OUT) :: Value

    Value=2.0_CMISSRP*DEXP(x)*DCOS(y)

  END SUBROUTINE AnalyticalSolution
